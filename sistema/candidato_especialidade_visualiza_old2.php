    ...
    <div class="row">
        <div class="col-md-12" >
            ...
            <div class="card" <?php if(!inscricao()) echo ' hidden ' ?>>
                <form action="../banco_dados/candidato_cadastra_especialidade.php" method="get" id="form_cadastro_especialidade">
                    ...
                    <div class="row">
                        <div class="form-group col-lg-6" id="div_ott_stt">
                            ...
                            <select id="ott_stt" name="ott_stt" class="form-control" onchange="carregarEspecialidades()">
                                ...
                                <?php
                                    ...
                                    if($codigo_selecao == "eipot " || $_SESSION['eipot'] == 1)
                        ...
                        <div class="form-group col-lg-6">
                            <button type="button" class="btn btn-primary" onclick="carregarEspecialidades()">Carregar Especialidades</button>
                        </div>
                            ...
                            <?php
                            // processa_busca_especialidades.php
                            if (isset($_GET['ott_stt']) && !empty($_GET['ott_stt'])) {
                                $ott_stt = $_GET['ott_stt']; // Obtém o valor enviado via GET
                                include_once '../../banco_dados/conexao.php';
                                $conexao = new Conexao();
                                
                                // Consulta para buscar as especialidades com base no tipo selecionado
                                $resultado = $conexao->get_especialidade_ott_stt($ott_stt); 
                                
                                // Verifica se existem especialidades e as exibe
                                if ($resultado) {
                                ...
                                echo '<option value="">Primeiramente selecione o tipo de especialidade</option>';
                            }
                            ?>
                                </select>
                            </div>
                            <script>
    // Função que carrega as especialidades com base no tipo selecionado
    function carregarEspecialidades() {
        var tipoEspecialidade = document.getElementById('ott_stt').value;

        if (tipoEspecialidade) {
            // Atualiza o valor do campo 'ott_stt' no formulário com o tipo selecionado
            var form = document.getElementById('form_cadastro_especialidade');
            var action = form.action; // Para preservar a ação do formulário (se necessário)
            var xhr = new XMLHttpRequest();
            xhr.open('GET', 'processa_busca_especialidades.php?ott_stt=' + tipoEspecialidade, true);
            xhr.onreadystatechange = function() {
                if (xhr.readyState === 4 && xhr.status === 200) {
                    // Atualiza o select de especialidade com as opções retornadas
                    document.getElementById('especialidade').innerHTML = xhr.responseText;
                }
            };
            xhr.send();
        } else {
            // Caso não tenha selecionado nada, mostra uma opção padrão
            document.getElementById('especialidade').innerHTML = '<option value="">Primeiramente selecione o tipo de especialidade</option>';
        }
    }
</script>
            ...
            <script>
            // Função que carrega as especialidades sem enviar o formulário
                function carregarEspecialidades() {
                    var tipoEspecialidade = document.getElementById('ott_stt').value;
                    ...
                    if (tipoEspecialidade) {
                        ...
                        xhr.onreadystatechange = function() {
                            if (xhr.readyState === 4 && xhr.status === 200) {
                                // Recebe a resposta e insere as opções no select
                                document.getElementById('especialidade').innerHTML = xhr.responseText;
                            }
                        };
                        xhr.send();
                    } else {
                        // Caso não tenha selecionado, exibe uma mensagem padrão
                        document.getElementById('especialidade').innerHTML = '<option value="">Primeiramente selecione o tipo de especialidade</option>';
                    }
                }
            </script>
