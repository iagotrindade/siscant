...
$questionario = $conexao->get_respostas_questionario_candidato($candidato[0]['id']);
...
<table border='0' style='width:100%; margin-top: 5px; margin-bottom: 5px;'>
    <tr>
        <th align='center'><strong>Questionário de Inscrição candidato " . $candidato[0]['nome_completo'] . "</strong></th>
    </tr>
</table>

<p style='font-size: 12px; text-align: justify; margin: 5px 0; text-indent: 2em;'>
    O candidato abaixo, preencheu o Questionário de Inscrição, conforme previsto em Aviso de Convocação, e sobre isto, inseriu as respostas abaixo:
</p> 
";
$mpdf->AddPage('P');
$mpdf->WriteHTML($html);
...
$linhasHtml = $linhasHtml . "
    <tr style='background-color: #D8D8D8'>
        <td colspan=\"2\"> <center><b>RESPOSTAS</b></center></td>
    </tr>
    </table>";

foreach ($questionario as $index => $pergunta) {
    $linhasHtml .= "

    <div style='margin-bottom: 20px;'>
    <h5 style='margin: 0 0 5px; font-size: 10px;'>
        <b>Pergunta Nº " . ($index + 1) . ":</b> " . $pergunta['texto_pergunta'] . "
    </h5>

    <p style='font-size: 10px; text-align: justify; margin: 5px 0;'>
        <b>Resposta:</b> " . $pergunta['resposta'] . "
    </p>
...
$cpf = mascara($candidato[0]['cpf'], '###.###.###-##');

$linhasHtml .= "
    <table border='0' style='font-size: 10px; font-family: Times New Roman; width:100%'>
        <tr style='background-color: #D8D8D8'>
            <td colspan=\"2\"> <center><b>Questionário enviado em " . trata_data($questionario[0]['data_resposta']) . " por " . $candidato[0]['nome_completo'] . ", inscrito no CPF " . $cpf . "</b></center></td>
        </tr>
    </table>
";
$contador++;
...
// Gera o PDF
$mpdf->Output("Questionário de Inscrição.pdf", 'D');
ob_end_flush();
exit;
