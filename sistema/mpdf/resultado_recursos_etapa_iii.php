...
<p style='font-size: 12px; text-align: justify; margin: 5px 0; text-indent: 2em;'>$paragrafo_um_resultado</p>
<p style='font-size: 12px; text-align: justify; margin: 5px 0; text-indent: 2em;'>$paragrafo_dois_resultado</p>
...
$existeObsEtapa = false;

foreach ($inscritos_por_arma as $candidatos) {
    foreach ($candidatos as $candidato) {
        if (isset($candidato['obs_etapa']) && $candidato['obs_etapa'] === '3 - IS') {
            $existeObsEtapa = true;
...
if ($existeObsEtapa) {
    ...
    <table border='0' style='width:100%; margin-top: 5px;'>
        <tr><th align='left'><strong>2. CONVOCAÇÃO PARA ISGRec (INSPEÇÃO DE SAÚDE EM GRAU DE RECURSO)</strong></th></tr>
    </table>
    
    <p style='font-size: 12px; text-align: justify; margin: 5px 0; text-indent: 2em;'>$paragrafo_um_convocacao</p>
    <p style='font-size: 12px; text-align: justify; margin: 5px 0; text-indent: 2em;'>$paragrafo_dois_convocacao</p>";
    ...
    foreach ($lista_candidatos_recurso as $inscrito) {
        if ($inscrito['etapa'] < 3 || $inscrito['status_final'] !== 'deferido') {
            continue;
        }
        if ($inscrito['rm_inscricao'] == $rm_usuario) { // Filtra por rm_inscricao
            $arma = $inscrito['arma_especialidade'];
    ...
    foreach ($inscritos_por_arma as $arma => $candidatos) {
        ...
        foreach ($candidatos as $candidato) {
            if ($candidato['status_final'] != 'deferido' || $candidato['obs_etapa'] != '3 - IS') {
                $esconder = true;
            }
        }
