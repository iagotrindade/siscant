...
foreach ($lista_especialidades as $especialidade) {
    $lista_candidatos = $conexao->get_candidatos_especialidade_desclassificados($especialidade['id']);
    ...
    foreach ($lista_candidatos as $candidato) {
        if ($candidato['etapa'] < 3 || $candidato['data_exame_saude_recurso'] == null) {
            continue;
        }

        if (!empty($data_inicial) && !empty($data_final)) {
            $data_inspecao = $candidato['data_exame_saude_recurso'];
        ...
        $aptoSaude = $statusMap[$candidato['apto_saude_recurso']] ?? 'DESCONHECIDO';
...
$mpdf->Output("Resultado Inspeção de Saúde em Grau de Recurso.pdf", 'D');
ob_end_flush();
exit();
