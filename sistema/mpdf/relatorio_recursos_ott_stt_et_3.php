...
// ÁREA DOS RECURSOS 3 - IS
foreach ($lista_candidatos_recurso as $recurso) {
    if ($recurso['etapa'] == 3 && $recurso['obs_etapa'] == '3 - IS' && $recurso['id_selecao'] == $_SESSION['selecao']) {
        $html = "
        <p style='font-size: 12px; text-align: justify; margin: 20px 0 5px 0; text-indent: 2em;'><strong>1. RESULTADO DA ANÁLISE DE RECURSO ETAPA III - JISE</strong></p>
    ...
    <table border='1' style='width:100%; border-collapse: collapse; margin-bottom: 20px;'>
        ...
        foreach ($lista_candidatos_recurso as $candidato) {
            ...
            if ($candidato['etapa'] != $etapa || $candidato['obs_etapa'] != '3 - IS' || $candidato['id_selecao'] != $_SESSION['selecao']) {
                continue;
            }
...
// ÁREA DOS RECURSOS 3 - DOCUMENTAL
foreach ($lista_candidatos_recurso as $recurso) {
    if ($recurso['etapa'] == 3 && $recurso['obs_etapa'] == '3 - Documental' && $recurso['id_selecao'] == $_SESSION['selecao']) {
        $html = "
        <p style='font-size: 12px; text-align: justify; margin: 20px 0 5px 0; text-indent: 2em;'><strong>2. RESULTADO DA ANÁLISE DE RECURSO ETAPA III - DOCUMENTAL</strong></p>
...
// ÁREA DA CONVOCAÇÃO PARA ISGREC
foreach ($lista_candidatos_recurso as $recurso) {
    if ($recurso['etapa'] == 3 && $recurso['obs_etapa'] == '3 - IS' && $recurso['status_final'] == 'deferido' && $recurso['id_selecao'] == $_SESSION['selecao']) {
        $html = "
        <p style='font-size: 12px; text-align: justify; margin: 20px 0 5px 0; text-indent: 2em;'><strong>3. CONVOCAÇÃO PARA ISGR (INSPEÇÃO DE SAÚDE EM GRAU DE RECURSO)</strong></p>
        ...
        <table border='1' style='width:100%; border-collapse: collapse; margin-bottom: 20px;'>
            ...
            <tr style='text-align: center; background-color: #D8D8D8; font-size: 12px;'>
                <th style='font-size: 12px; text-align: center; width: 5%;'>Nº</th>
                <th style='font-size: 12px; text-align: center; width: 10%;'>CPF</th>
                <th style='font-size: 12px; text-align: center; width: 85%;'>NOME</th>
            </tr>
        ";
...
$mpdf->Output("Resultado Análise Recursos Etapa III.pdf", 'D');
ob_end_flush();
exit();
