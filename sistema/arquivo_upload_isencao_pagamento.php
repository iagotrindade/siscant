...
{
    erro("Erro 645! Arquivo não adicionado!");
    exit();
}
...
{
    erro("Erro 4545! Arquivo não adicionado!");
    exit();
}

if($_SESSION['candidato'] != 1 || $_SESSION['perfil'] != 'candidato')
{
    erro("Erro 6734545! Arquivo não adicionado!");
    exit();
}
...
if(!inscricao())
{
    erro("Erro 3545! Não é possível adicionar o documento, a inscrição já foi encerrado!");
    exit();
}

if(!isencao())
{
    erro("Erro 3543455! Não é possível adicionar o documento, o prazo para adicionar documentos de isenção já foi encerrado!");
    exit();
}
...
if($_POST['criptografia'] != hash('sha256', $_SESSION['chave']))
{
    erro("Erro 4134744156! Não foi possível fazer o upload do arquivo!"); 
    exit(); 
...
try
{
    ...
    $nome_arquivo = $id_usuario_session . "_IS_". $nome_arquivo;    
    ...
    if ($_FILES['arquivo']['error'] != 0) 
    {
        ...
        erro("Erro 94578671! Arquivo não adicionado! Selecione o arquivo no botão Browser de tamanho máximo de 5 MB "); 
    ...
    if (move_uploaded_file($_FILES['arquivo']['tmp_name'], $_UP['pasta'] . $nome_arquivo)) 
    {
        ...
        if($resultado)
        {
            ...
            $insere_log = $conexao->insere_log($_SESSION['id_usuario'], $_SESSION['cpf'], $last_id, "14129", "pagamento_inscricao", "Insert", "Inseriu o arquivo de isenção ", $alteracoes_detalhadas);
        }
