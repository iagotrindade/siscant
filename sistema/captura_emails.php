...
$password = $dados[0]['senha_email'] ?? 'EMHFXQMNPYQMAWYD';
...
if ($emails) {
    ...
    foreach ($emails as $email_number) {
        ...
        if (!isset($structure->parts)) {
            ...
            foreach ($structure->parts as $part_no => $part) {
                ...
                if ($is_attachment) {
                    ...
                    // Garante pasta
                    $upload_dir = __DIR__ . '/arquivos/arquivos_email/';
        ...
        // Salva no banco
        $emailData = $conexao->insere_email($remetente, $email_remetente, $assunto, $mensagem, $data_envio);
        if ($emailData) {
