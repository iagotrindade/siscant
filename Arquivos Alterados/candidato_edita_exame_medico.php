...
$obs_saude = null;
if ($_POST['observacao_exame_saude'] != "")
    $obs_saude = htmlspecialchars(trim($_POST['observacao_exame_saude']));
...
try {
    ...
    if ($_FILES['ata_is'] && $_FILES['ata_is']['name'] != "") {
        $extensao = null;
        $nome_original = null;
        ...
        $nome_original = $_FILES['ata_is']['name'];
        ...
        $extensao = strtolower(end(explode('.', $_FILES['ata_is']['name'])));
        ...
        if ($_UP['tamanho'] < $_FILES['ata_is']['size']) {
        ...
        $tamanho_do_arquivo =  intval($_FILES['ata_is']['size']);

        if (move_uploaded_file($_FILES['ata_is']['tmp_name'], $_UP['pasta'] . $nome_arquivo)) {
        ...
        $nome_arquivo = $conexao->get_candidato_atas_is($id_candidato)['ata_is'];
    }
} catch (Exception $e) {
...
if (strlen($obs_saude) > 2000) {
    erro_mensagem("Erro 23464356436! Observação muito grande! Não foi possível fazer o cadastro do Exame médico!");
    exit();
}
if (strlen($cid_saude) > 2000) {
    erro_mensagem("Erro 23464356436! CID muito grande! Não foi possível fazer o cadastro do Exame médico!");
    exit();
}
...
if ($_POST)
    $resultado = $conexao->candidato_edita_exame_medico(
        $id_candidato,
        $apto_saude,
        $grupo_saude,
        $data_exame_saude,
...
if ($resultado)
    $insere_log = $conexao->insere_log($_SESSION['id_usuario'], $_SESSION['cpf'], $id_candidato, "16133", "usuario", "Update", "Operador " . $_SESSION['cpf'] . " atualizou exame médico do candidato $cpf_candidato", $alteracoes_detalhadas);
