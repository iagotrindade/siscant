...
if (
  ...
  //      $nome_social = htmlspecialchars(trim($_POST['nome_social']));
    ...
    if($_POST['nacionalidade'] != "")
        $nacionalidade = htmlspecialchars(trim($_POST['nacionalidade']));
   ...
   // var_dump($cidade); exit;
    ...
    if($_POST['autodeclaracao'] != "")
        $autodeclaracao  = htmlspecialchars(trim($_POST['autodeclaracao']));
    
    if($_POST['rm_inscricao'] != "" || $_POST['rm_inscricao'] == null)
        $rm_inscricao  = htmlspecialchars(trim($_POST['rm_inscricao']));

    if($_POST['rm_destino_valores'] != "" || $_POST['rm_destino_valores'] == null)
        $rm_destino  = htmlspecialchars(trim($_POST['rm_destino_valores']));
   
    if (isset($_POST['vaga_reservada']) && $_POST['vaga_reservada'] != "") {
        $vaga_reservada = isset($_POST['vaga_reservada']) ? 1 : 0;
        } else {
            $vaga_reservada = null; // Ou um valor padrão, caso o checkbox não esteja marcado
        }
    /* JA ESTAVA
...
//     echo (float)htmlspecialchars($_POST['nota_ofor']);
    if (isset($_POST['nota_ofor']) && $_POST['nota_ofor'] != "") 
        $nota_ofor = (float)trim($_POST['nota_ofor']);
      //  echo $nota_ofor; exit;
    ...
    */ //JA ESTAVA
    ...
    if(isset($_SESSION['eipot']) && $_SESSION['eipot'] === 1)
    {
      ...
      /* if($arma_eipot == "outra" || $arma_eipot == "")
        {
            erro("Erro 96396396396! Nenhuma outra arma além das disponibilizadas para seleção tem vagas disponíveis!");
            exit();
        } */
...
//var_dump($nome_completo); exit;
 /* 
if($_POST)
        $resultado = $conexao->edita_candidato_eipot($_SESSION['id_usuario'],
                                                 $nome_completo,
                                                $identidade,
                                                $data_nascimento);
                                              /*  $nome_social ,
