...
if($etapa == 2)
{
    ...
    {
        $resultado2 = $conexao->altera_etapa_candidato($candidato['id'],$etapa);
        ...
        if($resultado2)
        {
            ...
            if($resultado)
                $insere_log = $conexao->insere_log($_SESSION['id_usuario'], $candidato['cpf'], $candidato['id'], "14122", "usuario", "Insert", "Observação adicionada, Candidato passou para ETAPA II", $alteracoes_detalhadas);
        }
    }
    
    $resultado3 = $conexao->altera_etapa_selecao($etapa);
    if($resultado3)
    {
...
if($etapa > 2)
{
    ...
    if($resultado3)
    {
        ...
        $usuarios_passaram_etapa2 =  print_r($resultado3, true);
        $insere_log = $conexao->insere_log($_SESSION['id_usuario'], $_SESSION['cpf'], $_SESSION['id_usuario'], "16124", "usuario", "Update", "Passou a seleção para a ETAPA: $etapa", $usuarios_passaram_etapa2);
        $conexao = null;
        header ("Location: ../sistema/etapa_passagem.php?sucesso=1");
        exit();
