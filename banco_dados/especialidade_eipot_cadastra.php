...
if($usuario_logado[0]['codigo_selecao'] == "eipot" && $eipot == "")
{
    erro("Erro 2354! A seleção de EIPOT é obrigatória");
    exit();
}

if($eipot == "" || $eipot == null)
{
    erro("Erro 345345345! A seleção de EIPOT é obrigatória");
    exit();
}
    
$verifica_nome = $conexao->get_nome_especialidade($nome_especialidade, $eipot);
if(count($verifica_nome) > 0)
{
    erro("Erro 5345! A especialidade $eipot - $nome_especialidade já está cadastrada!");
    exit();
}

if($_POST)
    $resultado = $conexao->insere_especialidade_eipot($nome_especialidade, $teste_pratico, $eipot, $musica);
...
header("Location: ../sistema/especialidade_visualiza_eipot.php");
exit();
