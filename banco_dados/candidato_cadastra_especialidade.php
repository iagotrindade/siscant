...
if ($_POST['id_candidato'] != "")
    $id_candidato = htmlspecialchars($_POST['id_candidato']);
...
$verifica_candidato_especialidade = $conexao->verifica_especialidade_candidato($id_candidato, $especialidade);
...
if ($_POST)
    $resultado = $conexao->insere_especialidade_candidato($id_candidato, $especialidade, $registro_conselho, $data_habilitacao, 1);

$alteracoes_detalhadas =  print_r($resultado, true);

if ($resultado)
    $insere_log = $conexao->insere_log($id_candidato, $_SESSION['cpf'], null, "14109", "candidato_x_especialidade", "Insert", "Se inscreveveu na especialide " . $get_especialidade[0]['nome'], $alteracoes_detalhadas);

$conexao = null;

if($selecao[0]['codigo'] == 'cet' && $_SESSION['perfil'] == 'admin') {
    header("Location: ../sistema/usuario_visualiza.php?id_usuario=$id_candidato#admin_cadastra_especialidade");
} else {
