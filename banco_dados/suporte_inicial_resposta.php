...
if ($id_suporte != null && $resposta != null) {
    $lista_suporte = $conexao->get_suporte_candidato_nao_inscrito($cpf_requerente);
    ...
    foreach ($lista_suporte as $suporte) {
        if ($suporte['id'] != $id_suporte && $suporte['usuario_resposta'] != $_SESSION['id_usuario'] && empty($suporte['resposta'])) {
            $resultado = $conexao->insere_resposta_inicial($suporte['id'], '');
        }
    }
