...
$id_guarnicao = null;
if ($_POST['id_guarnicao'] != "")
    $id_guarnicao = htmlspecialchars(trim($_POST['id_guarnicao']));
...
$id_especialidade_selecioanda = null;
if ($_POST['id_especialidade'] != "")
    $id_especialidade_selecioanda = (int)htmlspecialchars(trim($_POST['id_especialidade']));
...
if ($_POST)
    $distribuidos = $conexao->get_candidatos_guarnicao($id_guarnicao);

foreach ($distribuidos as $distribuido) {
    $id_candidato = $distribuido['id_candidato'];
    $cpf_candidato = $distribuido['cpf'];
    $especialidade_incorporou = $distribuido['id_especialidade'];
    
    if (isset($id_especialidade_selecioanda) && $id_especialidade_selecioanda != null && $id_especialidade_selecioanda != $distribuido['id_especialidade']) {
        continue;
    }
...
header("Location: ../sistema/relatorio_distribuicao.php");
