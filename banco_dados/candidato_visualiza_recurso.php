...
if(!$_POST){
    erro_mensagem("Erro 462346346!");  // Exibe mensagem de erro e interrompe a execução
    exit();
}

// Verifica se as variáveis de sessão 'chave' e 'selecao' estão setadas
 if(!isset($_SESSION['chave']) || !isset($_SESSION['selecao'])){
    $conexao = null;// Fecha a conexão com o banco
    erro("Erro 34236346! Não foi possível atualizar as datas"); // Exibe mensagem de erro
    exit();
  }

// Verifica se o perfil do usuário não é 'admin' ou se o usuário é um candidato
if($_SESSION['perfil'] != 'admin' || $_SESSION['candidato'] == 1){
    $conexao = null;  // Fecha a conexão com o banco
    erro("Erro 3246346! Não foi possível atualizar as datas");  // Exibe mensagem de erro
    exit();
}
 /*
...
*/
 ...
 if($verifica_existencia) {
    ...
    exit();
        ...
        if($resultado) $insere_log = $conexao->insere_log($_SESSION['id_usuario'], $_SESSION['cpf'], $_SESSION['selecao'], "161502", "selecao", "Update", "Atualizou as datas para dar permissão para o candidato selecionar a recurso onde quer servir para: $ini até $fim", "$alteracoes_detalhadas");
