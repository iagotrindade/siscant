...
order by total_pontos_somados desc");
    ...
    public function get_especialidade_candidato($id_usuario)
    {
        $stmt = $this->pdo->prepare(
            "
                    select e.teste_pratico, e.nota_av, e.escolhe_guarnicao, e.id_candidato_escolhendo_guarnicao, ce.id id_candidato_x_especialidade, ce.cidade_escolheu_servir, ce.concorrendo, ce.justificativa, ce.id_especialidade, u.nome_completo, u.cpf, 
    ...
    public function reseta_especialidades_escolha_guarnicao()
    {
        $datetime   = date('Y-m-d H:i:s');
        $usuario    = $_SESSION['id_usuario'];
        $id_selecao = $_SESSION['selecao'];
        ...
        ";
            ...
            $stmt = $this->pdo->prepare($sql);
            $stmt->bindValue(':id_selecao', $id_selecao, PDO::PARAM_INT);
    ...
    public function insere_especialidades_guarnicao($especialidades)
    {
        $datetime   = date('Y-m-d H:i:s');
        $usuario    = $_SESSION['id_usuario'];
        $id_selecao = $_SESSION['selecao'];
        ...
        ";
            ...
            foreach ($especialidades as $id_especialidade) {
                ...
                $stmt->bindValue(':id', $id_especialidade, PDO::PARAM_INT);
                $stmt->bindValue(':id_selecao', $id_selecao, PDO::PARAM_INT);
                $stmt->bindValue(':data', $datetime);
                $stmt->bindValue(':usuario', $usuario, PDO::PARAM_INT);
            ...
            return [
                'id_selecao' => $id_selecao,
                'especialidades_atualizadas' => count($especialidades),
                '_data_ultima_atualizacao' => $datetime,
                '_usuario_ultima_atualizacao' => $usuario,
            ];
        } catch (Exception $e) {
    ...
    public function atualiza_status_feedback($id_feedback, $novo_status, $resposta = null)
    {
        $datetime = date('Y-m-d H:i:s');
        try {
