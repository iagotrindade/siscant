...
/*
$opcoes_selecionadas = isset($_POST['opcoes']) ? $_POST['opcoes'] : [];
foreach ($opcoes_selecionadas as $opcao) {
    // Verifica qual opção foi marcada e define a variável de sessão correspondente como true
    session_start(); // Inicia a sessão
    ...
    if ($opcao === 'dois_regiao') { 
        $segunda_regiao = 1;
    }
    
    if ($opcao === 'tres_regiao') { 
        $terceira_regiao = 1;
    }
    
    if ($opcao === 'quatro_regiao') { 
        $quarta_regiao = 1;
    }
    
    if ($opcao === 'cinco_regiao') { 
        $quinta_regiao = 1;
    }
    
    if ($opcao === 'seis_regiao') { 
        $sexta_regiao = 1;
    }
    
    if ($opcao === 'sete_regiao') { 
        $setima_regiao = 1;
    }
    
    if ($opcao === 'oito_regiao') { 
        $oitava_regiao = 1;
    }
    
    if ($opcao === 'nove_regiao') { 
        $nona_regiao = 1;
    }
    ...
    if ($opcao === 'onze_regiao') { 
        $onze_regiao = 1;
    }
    
    if ($opcao === 'doze_regiao') { 
        $doze_regiao = 1;
    }
