...
if ($tipo_proxima_vaga === 'AC') {
    ...
    if ($selecao[0]['codigo'] == 'ott_stt') {
        ...
        // Conta quantos cotistas já escolheram (independente do tipo de vaga)
        $cotistas_que_ja_escolheram = 0;
        for ($i = 0; $i < $escolheram; $i++) {
            if (isset($vetor_ordenado_candidatos[$i]) && $vetor_ordenado_candidatos[$i]['vaga_reservada'] == 1) {
                $cotistas_que_ja_escolheram++;
            }
        }

        // Se faltam cotistas para preencher as vagas CN
        if ($cotistas_que_ja_escolheram < $cns_ate_proxima) {
            ...
            // Se não encontrou cotista (todos já escolheram), pega o primeiro da fila
