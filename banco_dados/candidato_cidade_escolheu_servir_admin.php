...
foreach ($get_especialidade_candidato as $especialidade) {
    if ($id_especialidade == $especialidade['id_especialidade']) {
        ...
        if ($especialidade['id_candidato_escolhendo_guarnicao'] != $id_usuario) {
            erro("Erro 2473478458! Não esta liberado para você realizar a Escolha!");
            exit();
        }
    }
}
...
if ($cadastra_cidade_vai_servir) {
    ...
    if ($selecao[0]['codigo'] == 'ott_stt' && $usuario_eh_cotista) {
        $insere_log = $conexao->insere_log(
            ...
            "System",
            "Candidato escolheu Guarnicao",
            "Alterações detalhadas: $alteracoes_detalhadas "
        );
    }
...
header("Location: ../sistema/candidato_escolha_cidade_admin.php");
exit();
